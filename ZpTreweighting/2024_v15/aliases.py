...
# moved from LooseID to MediumID for new post-processed samples (DS, 23Apr26)
...
aliases['DY_NLO_ZpTrw'] = {
    'expr': '(' + DYrew['2024_v15']['LO_0j'].replace('x', 'gen_Zpt') + ')*(zeroJet)' +
            ' + (' + DYrew['2024_v15']['LO_1j'].replace('x', 'gen_Zpt') + ')*(oneJet&& Alt(CleanJet_pt,1,0)<30)' +
            ' + (' + DYrew['2024_v15']['LO_2j'].replace('x', 'gen_Zpt') + ')*(multiJet)',
    'samples': ['DY']
}
