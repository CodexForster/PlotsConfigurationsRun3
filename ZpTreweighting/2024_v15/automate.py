...
def wait_for_condor_jobs(batch_dir, tag, poll_interval=120, dry_run=False):
    """Holds execution until the user's summary line shows 0 idle and 0 running jobs."""
    CONDOR_REGISTRATION_DELAY = 30
    ...
    info(f"Waiting {CONDOR_REGISTRATION_DELAY}s for HTCondor to register jobs...")
    time.sleep(CONDOR_REGISTRATION_DELAY)

    banner(f"Monitoring summary counts for your username")
    
    # Get the current logged-in username (e.g., dshekar)
    username = os.environ.get("USER", "unknown")

    while True:
        # Run condor_q for your username
        rc, stdout, stderr = run_cmd_output(["condor_q", username])
        
        if rc != 0:
            info("WARNING: condor_q query failed. Retrying in 30 seconds...")
            time.sleep(30)
            continue
        ...
        # Scan for the specific user summary line
        for line in stdout.splitlines():
            if line.startswith(f"Total for {username}:"):
                user_line_found = True
                # Regex searches for: X idle, Y running
                match = re.search(r"(\d+)\s+idle,\s+(\d+)\s+running", line)
                if match:
                    idle = int(match.group(1))
                    running = int(match.group(2))
                    active_jobs = idle + running
                break

        # Fallback if your username wasn't printed (e.g., if the queue is completely empty)
        if not user_line_found:
            # Check the global query summary line instead
            for line in stdout.splitlines():
                if line.startswith("Total for query:"):
                    match = re.search(r"(\d+)\s+idle,\s+(\d+)\s+running", line)
                    if match:
                        active_jobs = int(match.group(1)) + int(match.group(2))
                    break

        if active_jobs == 0:
            info("Success: 0 idle and 0 running jobs detected. Proceeding...")
            return

        info(
            f"[{time.strftime('%H:%M:%S')}] "
            f"Detected {active_jobs} active task(s) in your queue. "
            f"Polling again in {poll_interval} s..."
        )
        time.sleep(poll_interval)
...
def phase2_extract(zptrw_dir, cfg, year="2022_v12", sample_type="LO", run_fit = "", dry_run=False):
...
def parse_args():
    ...
    parser.add_argument(
        "--year",
        default="2022_v12",
        help="Year key written to DYrew in dyZpTrw.json (default: '2022_v12').",
    )
    parser.add_argument(
        "--sample-type",
        default="LO",
